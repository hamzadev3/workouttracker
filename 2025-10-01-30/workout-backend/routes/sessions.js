...
// PATCH update exercise (sets/reps/weight/title) by index
...
// --- Seed realistic demo data (guarded) --------------------------------------
function rint(min, max){ return Math.floor(Math.random()*(max-min+1))+min; }
const round5 = n => Math.max(0, Math.round(n/5)*5);

// map a user's "strength" to typical lifts (very rough but believable)
...
// Allow in dev freely; in prod require x-admin-key header to match ADMIN_KEY
